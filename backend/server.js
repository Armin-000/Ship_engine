...
function findComponent(db, key, candidates = []) {
  ...
  for (const [dbKey, component] of Object.entries(db)) {
    ...
    const normalizedSfiaId = normalizeKey(component?.sfiaId || "");

    if (
      normalizedCandidates.includes(normalizedDbKey) ||
      normalizedCandidates.includes(normalizedSfiaId)
    ) {
      return {
        key: dbKey,
        component,
...
// Status platforme za login ekran
app.get("/api/status", (req, res) => {
  try {
    ...
    const componentsCount = entries.length;
    ...
    res.json({
      ...
      pdfDocuments: uniqueDocuments.size,
